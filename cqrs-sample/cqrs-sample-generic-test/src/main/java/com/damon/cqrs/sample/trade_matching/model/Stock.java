...
public class Stock extends AggregateRoot {
    ...
    private void apply(StockSelledEvent event) {
        if (event.isFinished()) {
            StockSellOrder sellOrder = sellOrderPriorityQueue.poll();
    ...
    private void apply(StockBoughtEvent event) {
        if (event.isFinished()) {
            StockBuyOrder buyOrder = buyOrderPriorityQueue.poll();
    ...
    public int match(StockMatchCmd cmd) {
        ...
        if (buyOrder.getNumber() < sellOrder.getNumber()) {
            applyNewEvent(new StockSelledEvent(
                    this.id, sellOrder.getPrice(), sellOrder.getOrderId(), sellOrder.getNumber(), buyOrder.getNumber(), false
            ));
            applyNewEvent(new StockBoughtEvent(
                    this.id, sellOrder.getPrice(), buyOrder.getOrderId(), buyOrder.getNumber(), buyOrder.getNumber(), true
            ));
        } else if (buyOrder.getNumber().equals(sellOrder.getNumber())) {
            applyNewEvent(new StockSelledEvent(
                    this.id, sellOrder.getPrice(), sellOrder.getOrderId(), sellOrder.getNumber(), sellOrder.getNumber(), true
            ));
            applyNewEvent(new StockBoughtEvent(
                    this.id, buyOrder.getPrice(), buyOrder.getOrderId(), buyOrder.getNumber(), buyOrder.getNumber(), true
            ));
        } else {
            applyNewEvent(new StockSelledEvent(
                    this.id, sellOrder.getPrice(), sellOrder.getOrderId(), sellOrder.getNumber(), sellOrder.getNumber(), true
            ...
            applyNewEvent(new StockBoughtEvent(
                    this.id, buyOrder.getPrice(), buyOrder.getOrderId(), buyOrder.getNumber(), buyOrder.getNumber(), false
            ));
        }
        return 0;
