        ...
        for (int i = 0; i < threadNumber; i++) {
            service.submit(() -> {
                for (int count = 0; count < exeCount; count++) {
                    ...
                    GoodsStockAddCommand command = new GoodsStockAddCommand(IdWorker.getId(), goodsIds.get(index));
                    CompletableFuture<Integer> future = handler.process(command, goods -> goods.addStock(1), 3);
                    try {
                        future.join();
                    } catch (Exception e) {
                        e.printStackTrace();
