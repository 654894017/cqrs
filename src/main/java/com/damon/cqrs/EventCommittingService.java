        ...
        eventStore.store(map).thenAccept(results -> {
            results.forEach(result -> {
                ...
                if (EventAppendStatus.Success.equals(result.getEventAppendStatus())) {
                    ...
                    try {
                        Class<T> aggreClass = ReflectUtis.getClass(group.getAggregateType());
                        domainService.getAggregateSnapshoot(group.getAggregateId(), aggreClass).whenComplete((as, e) -> {
                            if (e != null) {
                                ...
                                sourcingEvent(ReflectUtis.newInstance(aggreClass), 1, Integer.MAX_VALUE);
                            }
                        }).thenAccept(as -> {
                            if (as != null) {
                                sourcingEvent(as, as.getVersion() + 1, Integer.MAX_VALUE);
                            } else {
                                T newAggregate = ReflectUtis.newInstance(ReflectUtis.getClass(group.getAggregateType()));
